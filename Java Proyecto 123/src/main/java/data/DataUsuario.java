...
public class DataUsuario {
	...
	public boolean UserNameExist(String userName) throws SQLException {
		...
		try {
			stmt = DbConnector.getInstancia().getConn().prepareStatement(
					"Select count(id) as resultado From usuario Where usuario.nombre_usuario=? and habilitado=1");
...

	{
		...
		try {
			stmt = DbConnector.getInstancia().getConn().prepareStatement(
					"Select count(id) as resultado From usuario Where usuario.email=? and habilitado=1");
	...
	public LinkedList<Usuario> getAll() throws SQLException {
		...
		try {
			...
			rs = stmt.executeQuery(
					"select id,nombre_usuario,email,nickname,fecha_nacimiento,telefono,tipo,saldo from usuario where habilitado=1");
			if (rs != null) {
				while (rs.next()) {
	...
	public Usuario add(Usuario u) throws SQLException {
		...
		try {
			stmt = DbConnector.getInstancia().getConn().prepareStatement(
					"insert into usuario(nombre_usuario,email,contraseña,nickname,fecha_nacimiento,telefono,tipo,saldo) "
							+ "values(?,?,?,?,?,?,?,?)",
	...
	public void update(Usuario u)throws SQLException {
		PreparedStatement stmt = null;
		try {
			stmt = DbConnector.getInstancia().getConn().prepareStatement("update usuario set nombre_usuario=?,email=?,"
					+ "nickname=?,fecha_nacimiento=?,telefono=?,tipo=?,saldo=? where id=?");
	...
	public void updatePassword(Usuario u) throws SQLException {
		PreparedStatement stmt = null;
		try {
			stmt = DbConnector.getInstancia().getConn().prepareStatement("update usuario set contraseña=? where id=?");
